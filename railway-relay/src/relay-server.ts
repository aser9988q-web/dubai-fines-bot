...
app.get("/relay/health", function (_req, res) {
  res.json({ status: "ok", service: "dubai-police-relay", timestamp: new Date().toISOString() });
});

app.use("/relay", assertRelayToken);

app.post("/relay/scrape", async function (req, res) {
  const body = req.body ?? {};
  const plateSource = body.plateSource;
  const plateNumber = body.plateNumber;
  const plateCode = body.plateCode;
  try {
    const result = await scrapeDubaiFines(String(plateSource ?? ""), String(plateNumber ?? ""), String(plateCode ?? ""));
    res.json({ success: true, data: result });
  } catch (error) {
    res.status(502).json({ success: false, errorMessage: error instanceof Error ? error.message : "Relay scrape failed" });
  }
});

app.get("/relay/plate-data/:plateSrcCode", async function (req, res) {
  try {
    const response = await axios.get(DUBAI_POLICE_API + "/finespayment/getPlateData/" + encodeURIComponent(req.params.plateSrcCode), {
      headers: API_HEADERS,
      timeout: 15000,
      responseType: "text",
    ...
    res.status(502).json({ success: false, errorMessage: error instanceof Error ? error.message : "Relay upstream request failed" });
  }
});

app.post("/relay/search-fines", async function (req, res) {
  const body = req.body ?? {};
  try {
    const response = await axios.post(DUBAI_POLICE_API + "/finespayment/searchFines", {
      inquiryType: typeof body.inquiryType === "number" ? body.inquiryType : 3,
      plateNo: body.plateNo,
      plateCat: body.plateCat,
      plateSrcCode: body.plateSrcCode,
      plateCodeId: body.plateCodeId,
    }, {
      headers: API_HEADERS,
      timeout: 25000,
      responseType: "text",
